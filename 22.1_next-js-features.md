...
1. Importing image and using it
    ...
    - In Next.js we have to do the same import but while using it we have to use the `<ImageName>.src` property 
...
4. We cannot directly have element type of CSS in module CSS classes like `h1, div`. It needs to be in some other CSS class like

``` Not allowed.js
h1 {                        
  color: 'white'               // Not allowed
}
...
13. `not-found.js` file
    - Used to handle 404 errors on the sibling page or nested components
    - We can have a top level file that can cover all the components in app folder
...
14. Calling `not-found.js` programmatically
    - Example we're fetching some data from the database and it does not exist 
    - Then we can add an if condition and call the closes `not-found.js` page
