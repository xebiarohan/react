...
2. Accessing `loader` function return object using `useLoaderData`
   ...
   - We can directly use the `useLoaderData` to fetch data in the `EventsList` component as well as it is used in the `EventsPage`. Basically you can call it in the component where you are redirecting or any of its children.
...
16. `isRouteErrorResponse`
     - is a utility function from React Router used inside a `errorElement` (error boundary) to check whether the error came from a thrown Response in a loader or action.
     - It helps you distinguish between
        - HTTP-style errors (404, 401, 500) thrown using Response or json()
        - Normal JavaScript errors thrown using new Error().
